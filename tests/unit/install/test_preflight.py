...
def test_check_python_version_passes():
    with patch("piighost.install.preflight.sys.version_info", (3, 12, 0)):
        check_python_version()  # should not raise


def test_check_python_version_raises_for_old_python():
    with patch("piighost.install.preflight.sys.version_info", (3, 9, 0)):
        with pytest.raises(PreflightError, match="Python 3.12"):
            check_python_version()
